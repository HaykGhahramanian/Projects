...
@login_required
def delete():
    delete_letter = Users_Tweets.query.filter_by(user_id=current_user.id).first()
    if delete_letter:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if photo and allowed_file(photo.filename):
            ...
            photo_path = os.path.join(DevEnvConfig.UPLOAD_FOLDER, filename)
        ...
        new_user_tweets = Users_Tweets(user_id=current_user.id)
        db.session.add(new_user_tweets)
        db.session.commit()

        return redirect(url_for('profile'))

    return render_template('register.html', form=form, profile_image=photo_path)
...
def post_letter():
    if request.method == 'POST':
        ...
        _form = Users_Tweets(user_id=current_user.id, tweet_id=new_letter.id)
